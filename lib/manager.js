...
const { Worker: _Worker } = require('./worker_threads')

function initWorkers (opts, tracker, cb) {
  tracker = tracker || new EE()

  const Worker = opts.customWorker || _Worker
  const workers = []
  const results = []
  ...
  function init () {
    for (let i = 0; i < numWorkers; i++) {
      const worker = new Worker(path.resolve(__dirname, './worker.js'), { workerData: { id: i + 1, opts: workerOpts } })
